...
export class AvanceCreationComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    try{ this.personals = await this.listService.getAll(this.listService.list.USERS, {id: id_entite}).toPromise();} catch (e) {console.log('error filter FAMILY_SITUATION', e);}
  ...
  async createDemand() {
    ...
    try{
      ...
      this.messageService.add({severity: 'error', summary: 'Echec!', detail: 'Une erreur est survennue lors de la création',  sticky: false});
    }finally {
